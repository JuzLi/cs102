...
public interface VesselPreferenceRepository extends JpaRepository<VesselPreference, Integer> {
    ...
    @Transactional
    @Modifying
    @Query("delete from VesselPreference vp where vp.user = :user and vp.vessel = :vessel")
    void deleteVesselPreference(@Param("user") User user, @Param("vessel") Vessel vessel);
}
